...
class MonadTransControl t => PassesWriter t where
  ...
  fIn :: Monoid w => StT t (a, w -> w) -> (StT t a, w -> w)
...
instance
  ...
  where
  ask = lift ask
  local fn act = liftWith (\unlift -> local fn (unlift act)) >>= restoreT . return
...
instance
  ...
  listen (act :: Transformed t m a) =
    liftWith (\unlift -> listen (unlift act)) >>= restoreT . return . fOut @t @w @a
  pass (act :: Transformed t m (a, w -> w)) =
    liftWith (\unlift -> pass (fIn @t @w @a <$> unlift act)) >>= restoreT . return

instance (MonadTrans t, MonadState s m, Monad (t m)) => MonadState s (Transformed t m) where
  get = lift get
  put = lift . put
  state = lift . state

instance (MonadTransControl t, MonadError e m, Monad (t m)) => MonadError e (Transformed t m) where
  throwError = lift . throwError

  -- Loses state when an error is caught
  catchError act onError =
    liftWith (\unlift -> unlift act `catchError` \e -> unlift $ onError e) >>= restoreT . return
